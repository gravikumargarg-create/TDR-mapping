...
def _run_tdr_bml_merge(device_details_bytes, bml_excel_bytes):
    ...
    Build one Excel with: TDR sheet (copy), Pre-load device details (from device file, filtered), BML sheet (from BML file, filtered).
    Preserves formatting from the source files.
    ...
    try:
        ...
        return (None, str(e))
    # Find device details sheet (CUSTOMER_ID)
    dev_sheet_name = None
    customer_id_col = None
    ...
    # Build set of CUSTOMER_IDs from device details (header row 1)
    ...
    # Find TDR sheet in BML (name like TDR-* or first sheet with BAN in row 2)
    tdr_sheet_name = None
    tdr_ban_col = None
    tdr_header_row = 1
    ...
    # Collect all BANs from TDR sheet (data rows below header)
    ...
    for r in range(tdr_header_row + 1, ws_tdr.max_row + 1):
        ...
        for part in str(val).replace("\r", "\n").split("\n"):
            ...
            if bid:
                tdr_bans.add(bid)
    # Valid BANs = TDR BANs that exist in device details
    ...
    # Find Pre-load device details sheet name in BML (for reference; we'll write from device file)
    preload_name = "Pre-load device details"
    # Build output workbook
    from openpyxl import Workbook
    out = Workbook()
    out.remove(out.active)
    # 1) Copy TDR sheet from BML (with styles)
    ...
    # 2) Pre-load device details: from device file, rows where CUSTOMER_ID in valid_bans
    ...
    for r in range(2, ws_dev_src.max_row + 1):
        ...
        for c in range(1, ws_dev_src.max_column + 1):
            ...
            if cell_src.has_style:
                ...
                cell_out.alignment = copy(cell_src.alignment)
        out_row += 1
    # 3) BML sheet: from BML Excel, rows where BAN in valid_bans
...
def render_production():
    ...
    with col_clear:
        if st.button("Clear all", key="clear_full_btn", type="secondary", use_container_width=True, help="Clear LVT file, data files, optional fields, and results."):
            ...
            for k in ("lvt_result", "cap_validation_result", "cap_download", "cap_removed_bytes", "cap_highlighted_bytes", "cap_validation_key", "tdr_bml_result"):
    ...
    st.markdown(
        """
        <span style="font-weight: 700;">4. TDR + BML merge</span>
